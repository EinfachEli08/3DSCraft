#include "client/resources/data/AnimationMetadataSectionSerializer.h"
...
AnimationMetadataSection AnimationMetadataSectionSerializer::deserialize(cJSON* json) {
	...
	if (i != 1 && !isBetween(1, sizeof(u8), i))
	...
	if (JsonUtils::hasField(json, "frames")) {
		...
		u8 k = JsonUtils::getInt(json, "width", -1);
		u8 l = JsonUtils::getInt(json, "height", -1);

		if (k != -1 && !isBetween(1, sizeof(u8), k))
			Crash("Invalid width: %i\njson: %s", k, cJSON_Print(json));

		if (l != -1 && !isBetween(1, sizeof(u8), l))
...
AnimationFrame* AnimationMetadataSectionSerializer::parseAnimationFrame(u8 frame, cJSON* element) {
