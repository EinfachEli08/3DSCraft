...
void Inventory_Draw(int x, int y, ItemStack* stacks, int count) {
	...
	for (int i = 0; i < count; i++) {
		...
		if (Gui_EnteredCursorInside(headX * 2, headY * 2, 16 * 2, 16 * 2))
			clickAtStack(&stacks[i]);
		SpriteBatch_PushSingleColorQuad(headX * 2, headY * 2, 9, 16 * 2, 16 * 2, SHADER_RGB(20, 5, 2));
		even ^= true;
		headX += 16;
		if (headX >= rowInPixels) {
			headX = x;
			headY += 17;
			even = false;
			SpriteBatch_PushSingleColorQuad(x * 2, (headY - 1) * 2, 10, rowInPixels * 2, 2, SHADER_RGB(7, 7, 7));
		}
	}
