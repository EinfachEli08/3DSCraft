...
class meta_sequence_container {
	...
	public:
		...
		meta_sequence_container() noexcept : meta_sequence_container{locator<meta_ctx>::value_or()} {}

		/**
		 * @brief Context aware constructor.
		 * @param area The context from which to search for meta types.
		 */
		meta_sequence_container(const meta_ctx &area) noexcept : ctx{&area} {}
...
class meta_associative_container {
	...
	public:
		...
		/*! @brief Default constructor. */
		meta_associative_container() noexcept : meta_associative_container{locator<meta_ctx>::value_or()} {}

		/**
		 * @brief Context aware constructor.
		 * @param area The context from which to search for meta types.
		 */
		meta_associative_container(const meta_ctx &area) noexcept : ctx{&area} {}
...
class meta_any {
		...
		template <typename Type>
		static std::enable_if_t<std::is_same_v<std::remove_cv_t<std::remove_reference_t<Type>>, Type>> basic_vtable(
			[[maybe_unused]] const internal::meta_traits req, [[maybe_unused]] const bool const_only, [[maybe_unused]] const void *value,
				...
				if (req == internal::meta_traits::is_meta_pointer_like) {
					if constexpr (std::is_function_v<typename std::pointer_traits<Type>::element_type>) {
						...
						if constexpr (std::is_constructible_v<bool, Type>) {
							...
							static_cast<meta_any *>(other)->emplace<in_place_type>(
								adl_meta_pointer_like<Type>::dereference(*static_cast<const Type *>(value)));
						}
					}
				}
			}
			...
			if constexpr (is_complete_v<meta_sequence_container_traits<Type>>) {
				if (req == internal::meta_traits::is_meta_sequence_container) {
					const_only ? static_cast<meta_sequence_container *>(other)->rebind(*static_cast<const Type *>(value))
							   : static_cast<meta_sequence_container *>(other)->rebind(*static_cast<Type *>(const_cast<void *>(value)));
				}
			}

			if constexpr (is_complete_v<meta_associative_container_traits<Type>>) {
				if (req == internal::meta_traits::is_meta_associative_container) {
					const_only ? static_cast<meta_associative_container *>(other)->rebind(*static_cast<const Type *>(value))
							   : static_cast<meta_associative_container *>(other)->rebind(*static_cast<Type *>(const_cast<void *>(value)));
				}
			}
		}
	...
	public:
		...
		[[nodiscard]] meta_sequence_container as_sequence_container() noexcept {
			...
			vtable(internal::meta_traits::is_meta_sequence_container, policy() == meta_any_policy::cref, std::as_const(*this).data(),
				   &proxy);
			return proxy;
		}
		...
		[[nodiscard]] meta_associative_container as_associative_container() noexcept {
			...
			vtable(internal::meta_traits::is_meta_associative_container, policy() == meta_any_policy::cref, std::as_const(*this).data(),
				   &proxy);
			return proxy;
		}
		 ...
		 */
		[[nodiscard]] meta_any_policy policy() const noexcept { return storage.policy(); }
...
class meta_type {
		...
		[[nodiscard]] auto lookup(meta_any *const args, const typename internal::meta_type_node::size_type sz,
								  [[maybe_unused]] bool constness, Func next) const {
	...
	public:
		 ...
		 */
		[[nodiscard]] bool is_pointer() const noexcept { return static_cast<bool>(node.traits & internal::meta_traits::is_pointer); }
		 ...
		 */
		[[nodiscard]] meta_type remove_pointer() const noexcept { return {*ctx, node.remove_pointer(internal::meta_context::from(*ctx))}; }
		...
		[[nodiscard]] bool can_convert(const meta_type &other) const noexcept {
			return (internal::try_convert(internal::meta_context::from(*ctx), node, other.info(), other.is_arithmetic() || other.is_enum(),
										  nullptr,
										  [](const void *, auto &&...args) { return ((static_cast<void>(args), 1) + ... + 0u); }) != 0u);
		}
		...
		[[nodiscard]] meta_any construct(meta_any *const args, const size_type sz) const {
			if (node.details) {
				if (const auto *candidate = lookup(args, sz, false,
			...
			if (node.details) {
				if (auto it = node.details->func.find(id); it != node.details->func.cend()) {
					if (const auto *candidate =
							lookup(args, sz, instance && (instance->data() == nullptr),
...
[[nodiscard]] inline meta_any meta_any::allow_cast(const meta_type &type) const {
	return internal::try_convert(
		internal::meta_context::from(*ctx), node, type.info(), type.is_arithmetic() || type.is_enum(), data(),
		[this, &type]([[maybe_unused]] const void *instance, auto &&...args) {
			...
			} else if constexpr ((std::is_same_v<std::remove_const_t<std::remove_reference_t<decltype(args)>>,
...
class meta_sequence_container::meta_iterator final {
	...
	public:
		...
		meta_iterator() noexcept : meta_iterator{locator<meta_ctx>::value_or()} {}

		meta_iterator(const meta_ctx &area) noexcept : ctx{&area} {}

		template <typename It>
		meta_iterator(const meta_ctx &area, It iter) noexcept : ctx{&area}, vtable{&basic_vtable<It>}, handle{iter} {}
		...
		[[nodiscard]] const any &base() const noexcept { return handle; }
...
inline meta_sequence_container::iterator meta_sequence_container::insert(iterator it, meta_any value) {
	...
	if (const auto vtype = value_type_node(internal::meta_context::from(*ctx));
		!const_only &&
		(value.allow_cast({*ctx, vtype}) || value.allow_cast({*ctx, const_reference_node(internal::meta_context::from(*ctx))}))) {
		const bool is_value_type = (value.type().info() == *vtype.info);
		return insert_fn(*ctx, const_cast<void *>(data), is_value_type ? std::as_const(value).data() : nullptr,
						 is_value_type ? nullptr : std::as_const(value).data(), it);
	}
...
inline meta_associative_container::size_type meta_associative_container::erase(meta_any key) {
	return (!const_only && key.allow_cast(meta_type{*ctx, key_type_node(internal::meta_context::from(*ctx))}))
			   ? erase_fn(const_cast<void *>(data), std::as_const(key).data())
			   : 0u;
}
...
[[nodiscard]] inline meta_associative_container::iterator meta_associative_container::find(meta_any key) {
	return key.allow_cast(meta_type{*ctx, key_type_node(internal::meta_context::from(*ctx))})
			   ? find_fn(*ctx, const_only ? nullptr : const_cast<void *>(data), data, std::as_const(key).data())
			   : iterator{*ctx};
}
