...
namespace internal {
	...
	[[nodiscard]] inline const void *try_cast(const meta_context &context, const meta_type_node &from, const meta_type_node &to,
	...
	[[nodiscard]] inline auto try_convert(const meta_context &context, const meta_type_node &from, const type_info &to,
										  const bool arithmetic_or_enum, const void *instance, Func func) {
		...
		if (from.details) {
			...
			for (auto &&curr : from.details->base) {
				if (auto other = try_convert(context, curr.second.type(context), to, arithmetic_or_enum, curr.second.cast(instance), func);
					other) {
					return other;
				}
			}
	...
	[[nodiscard]] meta_type_node resolve(const meta_context &context) noexcept {
		...
		if constexpr (is_complete_v<meta_template_traits<Type>>) {
			node.templ =
				meta_template_node{meta_template_traits<Type>::args_type::size, &resolve<typename meta_template_traits<Type>::class_type>,
