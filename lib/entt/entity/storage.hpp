...
namespace internal {
	...
	class storage_iterator final {
		...
		public:
			...
			[[nodiscard]] constexpr pointer operator->() const noexcept { return std::addressof(operator[](0)); }
	...
	class extended_storage_iterator final {
		...
		public:
			...
			constexpr extended_storage_iterator(iterator_type base, Other... other) : it{base, other...} {}
...
class basic_storage : public basic_sparse_set<Entity, typename std::allocator_traits<Allocator>::template rebind_alloc<Entity>> {
	...
	protected:
		 ...
		 */
		underlying_iterator try_emplace([[maybe_unused]] const Entity entt, [[maybe_unused]] const bool force_back,
	...
	public:
		...
		explicit basic_storage(const allocator_type &allocator)
			: base_type{type_id<element_type>(), deletion_policy{traits_type::in_place_delete}, allocator}, payload{allocator} {}
		...
		basic_storage(basic_storage &&other, const allocator_type &allocator) noexcept
			...
			ENTT_ASSERT(alloc_traits::is_always_equal::value || get_allocator() == other.get_allocator(),
		 ...
		 */
		[[nodiscard]] iterable each() noexcept { return iterable{{base_type::begin(), begin()}, {base_type::end(), end()}}; }
...
class basic_storage<Type, Entity, Allocator, std::enable_if_t<component_traits<Type>::page_size == 0u>>
	...
	public:
		 ...
		 * @return An iterable object to use to _visit_ the storage.
		 */
		[[nodiscard]] iterable each() noexcept { return iterable{base_type::begin(), base_type::end()}; }

		/*! @copydoc each */
		[[nodiscard]] const_iterable each() const noexcept { return const_iterable{base_type::cbegin(), base_type::cend()}; }
		 ...
		 * @return A reverse iterable object to use to _visit_ the storage.
		 */
		[[nodiscard]] reverse_iterable reach() noexcept { return reverse_iterable{{base_type::rbegin()}, {base_type::rend()}}; }
...
class basic_storage<Entity, Entity, Allocator> : public basic_sparse_set<Entity, Allocator> {
	...
	public:
		 ...
		 */
		explicit basic_storage(const allocator_type &allocator)
			: base_type{type_id<void>(), deletion_policy::swap_only, allocator}, placeholder{} {}

		/**
		 * @brief Move constructor.
		 * @param other The instance to move from.
		 */
		basic_storage(basic_storage &&other) noexcept : base_type{std::move(other)}, placeholder{other.placeholder} {}
		 ...
		 */
		basic_storage(basic_storage &&other, const allocator_type &allocator) noexcept
			: base_type{std::move(other), allocator}, placeholder{other.placeholder} {}
		...
		entity_type emplace(const entity_type hint) {
			if (hint == null || hint == tombstone) {
				return emplace();
			} else if (const auto curr = traits_type::construct(traits_type::to_entity(hint), base_type::current(hint));
					   curr == tombstone) {
		 ...
		 * @return An iterable object to use to _visit_ the storage.
		 */
		[[nodiscard]] iterable each() noexcept { return std::as_const(*this).each(); }
		 ...
		 * @return A reverse iterable object to use to _visit_ the storage.
		 */
		[[nodiscard]] reverse_iterable reach() noexcept { return std::as_const(*this).reach(); }
