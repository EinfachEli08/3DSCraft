...
namespace internal {
	...
	class extended_group_iterator<It, owned_t<Owned...>, get_t<Get...>> {
		...
		public:
			...
			using value_type	  = decltype(std::tuple_cat(std::make_tuple(*std::declval<It>()), std::declval<Owned>().get_as_tuple({})...,
			...
			extended_group_iterator(iterator_type from, const std::tuple<Owned *..., Get *...> &cpools) : it{from}, pools{cpools} {}
			...
			[[nodiscard]] reference operator*() const noexcept {
				return std::tuple_cat(std::make_tuple(*it), index_to_element(*std::get<Owned *>(pools))...,
	...
	class group_handler final : public group_descriptor {
			...
			void push_on_construct(const entity_type entt) {
				if (std::apply(
						[entt, pos = len](auto *cpool, auto *...other) {
							return cpool->contains(entt) && !(cpool->index(entt) < pos) && (other->contains(entt) && ...);
						},
						pools) &&
...
class basic_group<owned_t<>, get_t<Get...>, exclude_t<Exclude...>> {
	...
	public:
		...
		static id_type group_id() noexcept {
			return type_hash<
				basic_group<owned_t<>, get_t<std::remove_const_t<Get>...>, exclude_t<std::remove_const_t<Exclude>...>>>::value();
		}
		...
		void sort(Compare compare, Sort algo = Sort{}, Args &&...args) {
			if (*this) {
				if constexpr (sizeof...(Index) == 0) {
					...
					auto comp = [&compare, cpools = pools_for(std::index_sequence_for<Get...>{})](const entity_type lhs,
...
class basic_group<owned_t<Owned...>, get_t<Get...>, exclude_t<Exclude...>> {
		...
		static_assert(!std::disjunction_v<std::bool_constant<component_traits<typename Owned::value_type>::in_place_delete>...>,
		...
		static constexpr std::size_t index_of =
			type_list_index_v<std::remove_const_t<Type>,
							  type_list<typename Owned::element_type..., typename Get::element_type..., typename Exclude::element_type...>>;
		...
		auto pools_for(std::index_sequence<Index...>, std::index_sequence<Other...>) const noexcept {
			...
			return descriptor ? return_type{static_cast<Owned *>(descriptor->template storage<Index>())...,
											static_cast<Get *>(descriptor->template storage<sizeof...(Owned) + Other>())...}
							  : return_type{};
		}
	...
	public:
		 ...
		 */
		static id_type group_id() noexcept {
			return type_hash<basic_group<owned_t<std::remove_const_t<Owned>...>, get_t<std::remove_const_t<Get>...>,
										 exclude_t<std::remove_const_t<Exclude>...>>>::value();
		}
