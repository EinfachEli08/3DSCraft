...
namespace internal {
	...
	template <typename Result, typename View, typename Other, std::size_t... VGet, std::size_t... VExclude, std::size_t... OGet,
			  std::size_t... OExclude>
	[[nodiscard]] Result view_pack(const View &view, const Other &other, std::index_sequence<VGet...>, std::index_sequence<VExclude...>,
								   std::index_sequence<OGet...>, std::index_sequence<OExclude...>) {
		Result elem{};
	...
	class view_iterator final {
			...
			[[nodiscard]] bool valid(const typename iterator_traits::value_type entt) const noexcept {
				return ((Get != 1u) || (entt != tombstone)) && internal::all_of(pools.begin(), pools.begin() + index, entt) &&
		...
		public:
			...
			constexpr view_iterator() noexcept : it{}, last{}, pools{}, filter{}, index{} {}

			view_iterator(iterator_type first, std::array<const Type *, Get> value, std::array<const Type *, Exclude> excl,
						  const std::size_t idx) noexcept
				: it{first}, last{value[idx]->end()}, pools{value}, filter{excl}, index{idx} {
	...
	struct extended_view_iterator final {
			...
			constexpr extended_view_iterator() : it{} {}

			extended_view_iterator(iterator_type from) : it{from} {}
				...
				return std::apply(
					[entt = *it](auto *...curr) {
						return std::tuple_cat(
							std::make_tuple(entt),
							static_cast<Type *>(const_cast<constness_as_t<typename Type::base_type, Type> *>(curr))->get_as_tuple(entt)...);
					},
					it.pools);
			}
...
class basic_common_view {
		template <typename Return, typename View, typename Other, std::size_t... VGet, std::size_t... VExclude, std::size_t... OGet,
				  std::size_t... OExclude>
		friend Return internal::view_pack(const View &, const Other &, std::index_sequence<VGet...>, std::index_sequence<VExclude...>,
										  std::index_sequence<OGet...>, std::index_sequence<OExclude...>);
	...
	protected:
		...
		basic_common_view(std::array<const Type *, Get> value, std::array<const Type *, Exclude> excl) noexcept
			: pools{value}, filter{excl}, index{Get} {
			unchecked_refresh();
		}

		void use(const std::size_t pos) noexcept { index = (index != Get) ? pos : Get; }
	...
	public:
		 ...
		 */
		[[nodiscard]] const common_type *handle() const noexcept { return (index != Get) ? pools[index] : nullptr; }

		/**
		 * @brief Estimates the number of entities iterated by the view.
		 * @return Estimated number of entities iterated by the view.
		 */
		[[nodiscard]] size_type size_hint() const noexcept { return (index != Get) ? pools[index]->size() : size_type{}; }
		...
		[[nodiscard]] iterator begin() const noexcept {
			return (index != Get)
					   ? iterator{pools[index]->end() - static_cast<typename iterator::difference_type>(offset()), pools, filter, index}
					   : iterator{};
		}
		...
		[[nodiscard]] bool contains(const entity_type entt) const noexcept {
			return (index != Get) && internal::all_of(pools.begin(), pools.end(), entt) &&
				   internal::none_of(filter.begin(), filter.end(), entt) && pools[index]->index(entt) < offset();
		}
...
class basic_view<get_t<Get...>, exclude_t<Exclude...>>
	: public basic_common_view<std::common_type_t<typename Get::base_type..., typename Exclude::base_type...>, sizeof...(Get),
							   sizeof...(Exclude)> {
		using base_type = basic_common_view<std::common_type_t<typename Get::base_type..., typename Exclude::base_type...>, sizeof...(Get),
											sizeof...(Exclude)>;

		template <typename Type>
		static constexpr std::size_t index_of =
			type_list_index_v<std::remove_const_t<Type>, type_list<typename Get::element_type..., typename Exclude::element_type...>>;
		...
		void each(Func &func, std::index_sequence<Index...>) const {
			for (const auto curr : storage<Curr>()->each()) {
				if (const auto entt = std::get<0>(curr); ((sizeof...(Get) != 1u) || (entt != tombstone)) &&
														 ((Curr == Index || this->pools[Index]->contains(entt)) && ...) &&
														 internal::none_of(this->filter.begin(), this->filter.end(), entt)) {
					if constexpr (is_applicable_v<Func, decltype(std::tuple_cat(std::tuple<entity_type>{},
	...
	public:
		 ...
		 */
		basic_view(Get &...value, Exclude &...excl) noexcept : base_type{{&value...}, {&excl...}} {}
		 ...
		 */
		[[nodiscard]] iterable each() const noexcept { return iterable{base_type::begin(), base_type::end()}; }
		...
		[[nodiscard]] auto operator|(const basic_view<get_t<OGet...>, exclude_t<OExclude...>> &other) const noexcept {
			return internal::view_pack<basic_view<get_t<Get..., OGet...>, exclude_t<Exclude..., OExclude...>>>(
				*this, other, std::index_sequence_for<Get...>{}, std::index_sequence_for<Exclude...>{}, std::index_sequence_for<OGet...>{},
				std::index_sequence_for<OExclude...>{});
		}
};
...
class basic_storage_view {
	protected:
		...
		basic_storage_view(const Type *value) noexcept : leading{value} {}
		/*! @endcond */
	...
	public:
		 ...
		 * @return The leading storage of the view.
		 */
		[[nodiscard]] const common_type *handle() const noexcept { return leading; }

		/**
		 * @brief Returns the number of entities that have the given element.
		 * @return Number of entities that have the given element.
		 */
		[[nodiscard]] size_type size() const noexcept { return leading ? leading->size() : size_type{}; }

		/**
		 * @brief Checks whether a view is empty.
		 * @return True if the view is empty, false otherwise.
		 */
		[[nodiscard]] bool empty() const noexcept { return !leading || leading->empty(); }
		 ...
		 * @return An iterator to the first entity of the view.
		 */
		[[nodiscard]] iterator begin() const noexcept { return leading ? leading->begin() : iterator{}; }

		/**
		 * @brief Returns an iterator that is past the last entity of the view.
		 * @return An iterator to the entity following the last entity of the view.
		 */
		[[nodiscard]] iterator end() const noexcept { return leading ? leading->end() : iterator{}; }
		 ...
		 */
		[[nodiscard]] reverse_iterator rbegin() const noexcept { return leading ? leading->rbegin() : reverse_iterator{}; }
		 ...
		 * reversed view.
		 */
		[[nodiscard]] reverse_iterator rend() const noexcept { return leading ? leading->rend() : reverse_iterator{}; }
		 ...
		 * otherwise.
		 */
		[[nodiscard]] entity_type front() const noexcept { return empty() ? null : *leading->begin(); }
		 ...
		 * otherwise.
		 */
		[[nodiscard]] entity_type back() const noexcept { return empty() ? null : *leading->rbegin(); }
		 ...
		 * iterator otherwise.
		 */
		[[nodiscard]] iterator find(const entity_type entt) const noexcept { return leading ? leading->find(entt) : iterator{}; }

		/**
		 * @brief Checks if a view is fully initialized.
		 * @return True if the view is fully initialized, false otherwise.
		 */
		[[nodiscard]] explicit operator bool() const noexcept { return (leading != nullptr); }
		 ...
		 * @return True if the view contains the given entity, false otherwise.
		 */
		[[nodiscard]] bool contains(const entity_type entt) const noexcept { return leading && leading->contains(entt); }
...
class basic_view<get_t<Get>, exclude_t<>, std::void_t<std::enable_if_t<!component_traits<typename Get::value_type>::in_place_delete>>>
	: public basic_storage_view<typename Get::base_type> {
	...
	public:
		 ...
		 */
		basic_view(Get &value) noexcept : base_type{&value} {}
		 ...
		 */
		[[nodiscard]] decltype(auto) operator[](const entity_type entt) const { return storage()->get(entt); }
		...
		[[nodiscard]] auto operator|(const basic_view<get_t<OGet...>, exclude_t<OExclude...>> &other) const noexcept {
			return internal::view_pack<basic_view<get_t<Get, OGet...>, exclude_t<OExclude...>>>(
				*this, other, std::index_sequence_for<Get>{}, std::index_sequence_for<>{}, std::index_sequence_for<OGet...>{},
				std::index_sequence_for<OExclude...>{});
		}
};
