...
class basic_sparse_set {
	...
	public:
		 ...
		 * @param allocator The allocator to use (possibly default-constructed).
		 */
		explicit basic_sparse_set(const type_info &elem, deletion_policy pol = deletion_policy::swap_and_pop,
								  const allocator_type &allocator = {})
			: sparse{allocator}, packed{allocator}, info{&elem}, mode{pol}, head{policy_to_head()} {
		 ...
		 */
		[[nodiscard]] bool contiguous() const noexcept { return (mode != deletion_policy::in_place) || (head == max_size); }
		 ...
		 * iterator otherwise.
		 */
		[[nodiscard]] const_iterator find(const entity_type entt) const noexcept { return contains(entt) ? to_iterator(entt) : end(); }
