...
#include "world/worldgen/artefacts/GenFunction.h"
...
void NormalGen::chunkFunction(WorkQueue* queue, WorkerItem item) {
	...
	for (int x = 0; x < CHUNK_SIZE; x++) {
		for (int z = 0; z < CHUNK_SIZE; z++) {
			...
			for (int y = 0; y < height - 3; y++) { Chunk_SetBlock(item.chunk, x, y, z, Block_Bedrock); }
			for (int y = 1; y < height - 3; y++) { Chunk_SetBlock(item.chunk, x, y, z, Block_Stone); }
			for (int y = height - 3; y < height; y++) { Chunk_SetBlock(item.chunk, x, y, z, Block_Dirt); }
