		...
		for (int i = 0; i < 24; i++) {
			...
			float3 dist =
				f3_sub(f3_new(newX * CHUNK_SIZE + CHUNK_SIZE / 2, newY * CHUNK_SIZE + CHUNK_SIZE / 2, newZ * CHUNK_SIZE + CHUNK_SIZE / 2),
					   playerPos);
			if (f3_dot(dist, dist) > (3.f * CHUNK_SIZE) * (3.f * CHUNK_SIZE)) { continue; }

			if (clusterWasRendered(newX, newY, newZ) & 1) continue;

			if (!ChunkCanBeSeenThrough(cluster->seeThrough, step.enteredFrom, dir) && step.enteredFrom != Direction_Invalid) continue;
	...
	for (int x = 1; x < CHUNKCACHE_SIZE - 1; x++) {
		for (int z = 1; z < CHUNKCACHE_SIZE - 1; z++) {
			...
			if ((chunk->revision != chunk->displayRevision || chunk->forceVBOUpdate) && !chunk->tasksRunning) {
				...
				if (clear) workqueue->addItem((WorkerItem){Enum::WorkerItemType::PolyGen, chunk});
			}
		}
	}
