...
const double omega_x = 1.0; // degrees per second for X-axis rotation
const double amplitude_y = 5.0; // degrees for Y-axis oscillation
const double frequency_y = 0.2; // oscillations per second for Y-axis
double t = 0.0;
...
void Renderer_Render() {
	...
	for (int i = 0; i < 2; i++) {
		...
		if (*gamestate == GameState_Playing) {
            ...
            angleX = C3D_AngleFromDegrees(amplitude_y * sin(frequency_y * t / 2));
            angleY = C3D_AngleFromDegrees(omega_x * t);
            BG_Render();
            ...
            SpriteBatch_PushText(210 - ((strlen("Surreal DEV") * 8)/2), 112, 0, INT16_MAX, false, INT_MAX, NULL, "Surreal DEV");
