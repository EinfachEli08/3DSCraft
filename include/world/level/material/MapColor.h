...
static constexpr u8 BRIGHTNESS_LEVELS[] = {180, 220, 255, 135};
static constexpr u32 MATERIAL_COLORS[]	= {
	 0,			// MapColor::NONE
...
static u32 calculateRGBColor(u8 colorId, u8 brightnessId) {
	if (colorId == NONE) {
		return 0;
	} else {
		u32 col	  = MATERIAL_COLORS[colorId];
		u8 bright = BRIGHTNESS_LEVELS[brightnessId];
		u16 r	  = (col >> 16 & 255) * bright / 255;
		u16 g	  = (col >> 8 & 255) * bright / 255;
		u16 b	  = (col & 255) * bright / 255;
		return -16777216 | r << 16 | g << 8 | b;
	}
}
static u32 getColorFromPackedId(u32 value) {
	u8 i = value & 255;
	return calculateRGBColor(i >> 2, i & 3);
}
};	// namespace MapColor
